...
ACSCL_x64 = os.path.join(my_path, "ACSCL_x64.dll")

# Import the ACS C library DLL
if platform.architecture()[0] == "32bit":
    acs = ctypes.windll.LoadLibrary('ACSCL_x86.dll')
if platform.architecture()[0] == "64bit":
    acs = ctypes.windll.LoadLibrary(ACSCL_x64)
...
def waitMotionEnd(hcomm, axis, timeout=INFINITE):
    call_acsc(acs.acsc_WaitMotionEnd, hcomm, int32(axis),
              int32(timeout))


# The function waits for the logical end of a motion.
def waitLogicalMotionEnd(hcomm, axis, timeout=INFINITE):
    call_acsc(acs.acsc_WaitLogicalMotionEnd, hcomm, int32(axis),
              int32(timeout))
...
def track(hcomm, flags, axis, wait=SYNCHRONOUS):
    call_acsc(hcomm, flags, axis, wait)
